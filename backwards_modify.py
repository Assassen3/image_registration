...
# 指定输入图片所在的目录
input_dir = ".\\data\\tomato_modified"

# 指定输出图片的目录
output_dir = ".\\data\\tomato_back_modified"
...
# 获取输入目录中的所有图片文件
image_files_ms = [f[:-9] for f in os.listdir(input_dir) if f.endswith("_ms_1.png")]

progress_bar_ms = tqdm(total=len(image_files_ms), unit="image")

for filename in image_files_ms:
    filepath = os.path.join(input_dir, filename + "_ms_1.png")
    ...
    with Image.open(filepath) as ms_img:
        ...
        output_filepath = os.path.join(output_dir, filename + "_ms_1.png")
